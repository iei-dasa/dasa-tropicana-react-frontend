...
export const fetchPerfumesByQuery = () => async (dispatch: Dispatch) => {
    ...
    dispatch(fetchPerfumesByQuerySuccess(response.data));
};

export const fetchPerfumeByQuery = (id: string) => async (dispatch: Dispatch) => {
    const response = await axios.get(API_BASE_URL +  "/perfume/" + id );
    dispatch(fetchPerfumeByQuerySuccess(response.data));
};
